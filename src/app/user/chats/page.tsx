...
export default function ChatWindow() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      const res = await axios.post(`/api/v1/chats/${chatId}/messages`, {
        content: newMessage,
      });
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white p-4 border-b flex items-center justify-between shadow-sm">
        ...
        {chatInfo?.property && (
          <button 
            onClick={() => router.push(`/properties/${chatInfo?.property?._id}`)}
            className="hidden sm:flex items-center space-x-2 p-2 rounded-lg hover:bg-gray-100"
          >
            <img
              src={chatInfo.property.images[0]?.url || '/placeholder-property.jpg'}
              alt={chatInfo.property.title}
              className="h-8 w-8 rounded-md object-cover"
            />
            <span className="text-sm font-medium">{chatInfo.property.title}</span>
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center p-8">
            ...
            <p className="text-gray-500 mt-2">Start the conversation with {otherParticipant?.name || 'the other user'}</p>
          </div>
        ) : (
